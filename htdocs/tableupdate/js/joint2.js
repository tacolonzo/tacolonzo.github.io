...
jQuery(function ($){
	var $modal = $('#editor-modal'),
		...
		ft = FooTable.init('#showcase-example-1', {
			columns: $.ajax("/tableupdate/content/columns.json", {dataType:'json', timeout: 3000}), //no cache for collumn because it shouldn't change
			rows: $.ajax("/tableupdate/content/rows.json", {cache: false, dataType:'json', timeout: 3000}),//this cache line is used so that the website wont keep the rows so it stays updated
			editing: {
				enabled: true,
				addRow: function(){
